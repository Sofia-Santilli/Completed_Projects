...
class MyWebSocketServer(tornado.websocket.WebSocketHandler):
    ...
    def on_message(self, data):
        ...
        if(received[0]=="doQuiz" or received[0]=="goDirectlyGame"):
...
def main():
    ...
    ''',
		ssl_options = {
		"certfile": os.path.join("host.cert"),
		"keyfile": os.path.join("host.key"),
		}
	)
    '''
    ...
    # Run web server for Pepper
    application_2 = tornado.web.Application([(r'/websocketserver', MyWebSocketServer2),])  
    http_server_2 = tornado.httpserver.HTTPServer(application_2)
    http_server_2.listen(server_port_2)
    print("%sWebsocket server for Pepper listening on port %d%s" %(GREEN,server_port_2,RESET))
